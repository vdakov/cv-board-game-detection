...
def board_detection_step(img_path, model, show_results=True, ground_truth_bbox=None):
    ...
    if results and hasattr(results[0], "boxes") and len(results[0].boxes) > 0:
        ...
        if show_results and ground_truth_bbox:
            ...
            utils.compare_prediction_ground_truth(img.copy(), img_with_bbox_rgb, ground_truth_img)
        elif show_results: 
            utils.show_before_and_after(img.copy(), img_with_bbox_rgb, title_before="Before Object Detection", title_after="After YOLO")
