...
// Store the selected image canvas for each tile
var tileToImageCanvas = {};
...
function loadImages(callback) {
	...
	// Scan for available images using a pattern: resourceType_X.png
	// where X is a number starting from 1
	for (var resourceType in resourceTypeToImageCanvas) {
		imgPaths[resourceType] = [];
		// Default image (original behavior)
		imgPaths[resourceType].push("images/" + resourceType + ".png");
		for (var i = 1; i <= 3; i++) {
			imgPaths[resourceType].push("images/" + resourceType + "_" + i + ".png");
		}
	}

	var totalImages = 0;
	for (var key in imgPaths) {
		totalImages += imgPaths[key].length;
	}
	...
	// Load all images for each resource type
	for (var resourceType in imgPaths) {
		for (var i = 0; i < imgPaths[resourceType].length; i++) {
			var path = imgPaths[resourceType][i];
			var img = new Image();
			img.src = path;

			// Use closures to capture the correct resourceType
			(function (rt, image, imagePath) {
				image.onload = function () {
					...
					resourceTypeToImageCanvas[rt].push(imgCanvas);
					loadedImages++;
					onAllImagesLoaded();
					console.log("Loaded: " + imagePath + " for " + rt);
				};

				image.onerror = function () {
					// Don't log for variant images that might not exist
...
CatanMap.prototype.generate = function () {
	...
	if (this.mapDefinition) {
		...
		// Clear the tile-to-image mapping
		tileToImageCanvas = {};
		...
		for (var i = 0; i < numDeserts; i += 1) {
			...
			// Select a random image for this tile if multiple are available
			if (resourceTypeToImageCanvas["desert"].length > 0) {
				var randomImageIndex = Math.floor(Math.random() * resourceTypeToImageCanvas["desert"].length);
				tileToImageCanvas[newCoords.toString()] = resourceTypeToImageCanvas["desert"][randomImageIndex];
			}
		...
		for (var i = 0; i < (numTiles - numDeserts); i += 1) {
			...
			// Select a random image for this tile if multiple are available
			if (resourceTypeToImageCanvas[newHexTile.resourceType].length > 0) {
				var randomImageIndex = Math.floor(Math.random() * resourceTypeToImageCanvas[newHexTile.resourceType].length);
				tileToImageCanvas[newCoords.toString()] = resourceTypeToImageCanvas[newHexTile.resourceType][randomImageIndex];
			}
...
HexTile.prototype.drawBase = function () {
	...
	if (mapStyle == "retro") {
		// Get the tile's coordinates as a string to look up in the tileToImageCanvas map
		var tileCoord = [this.gridX, this.gridY].toString();
		var imgCanvas;

		// Use the assigned image if one exists, otherwise fall back to the first image
		if (tileToImageCanvas[tileCoord]) {
			imgCanvas = tileToImageCanvas[tileCoord];
		} else if (resourceTypeToImageCanvas[this.resourceType].length > 0) {
			imgCanvas = resourceTypeToImageCanvas[this.resourceType][0];
		} else {
...
// Function to add additional image variants dynamically
function addImageVariant(resourceType, imagePath, callback) {
	if (!resourceTypeToImageCanvas[resourceType]) {
	...
	img.onload = function () {
		...
		resourceTypeToImageCanvas[resourceType].push(imgCanvas);
		console.log("Added new variant for " + resourceType + ": " + imagePath);
...
// Add a method to HexTile to set a specific image variant
HexTile.prototype.setImageVariant = function (variantIndex) {
	// Check if the resource type is valid and has the requested variant
	if (resourceTypeToImageCanvas[this.resourceType] &&
		resourceTypeToImageCanvas[this.resourceType].length > 0) {

		// Ensure the index is within bounds
		var index = variantIndex;
		if (index >= resourceTypeToImageCanvas[this.resourceType].length) {
			index = 0;
			console.warn("Requested variant index " + variantIndex +
				" exceeds available variants for " + this.resourceType +
				". Using variant 0 instead.");
		} else if (index < 0) {
			index = 0;
		...
		// Store the selected image canvas for this tile
		var tileCoord = [this.gridX, this.gridY].toString();
		tileToImageCanvas[tileCoord] = resourceTypeToImageCanvas[this.resourceType][index];
...
// Add a method to CatanMap to set a specific image variant for a specific tile
CatanMap.prototype.setTileImageVariant = function (gridX, gridY, variantIndex) {
	var tileCoord = [gridX, gridY].toString();
	var tile = this.coordToTile[tileCoord];

	if (tile) {
		return tile.setImageVariant(variantIndex);
	}

	return false;
}

// Add a method to get the number of available variants for a resource type
function getAvailableVariants(resourceType) {
	if (resourceTypeToImageCanvas[resourceType]) {
		return resourceTypeToImageCanvas[resourceType].length;
	}
	return 0;
}
